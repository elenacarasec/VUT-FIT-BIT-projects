...
**/
	...
	if (RootPtr != NULL) {
		if (RootPtr->Key != K) {
			found = BSTSearch(RootPtr->LPtr, K, Content);
			if (found == FALSE) {
				found = BSTSearch(RootPtr->RPtr, K, Content);
			}
		}
		else {
...
**/
	...
	if (newNode != NULL) {
		if (newNode->Key == K){
			...
			if (newNode->LPtr != NULL)
				BSTInsert(&(newNode->LPtr), K, Content);
			else
			{
			...
			if (newNode->RPtr != NULL)
				BSTInsert(&(newNode->RPtr), K, Content);
			else {
				tBSTNodePtr tmp = malloc(sizeof(struct tBSTNode));
				if (tmp) {
					tmp->Key = K;
