...
/*                                                                            */
/*  Reseni vytvoril(a) a odevzdava: (Elena Carasec, xcaras00)                 */
/*                                                                            */
/******************************************************************************/
...
void PORTB_IRQHandler(void) {
	...
	delay(2000);
	if (PORTB->ISFR == BUTTON_UP_MASK && !(GPIOB->PDIR & BUTTON_UP_MASK)) {
		flash();
	}
	else if (PORTB->ISFR == BUTTON_DOWN_MASK && !(GPIOB->PDIR & BUTTON_DOWN_MASK)) {
		beep();
	}
	else if (PORTB->ISFR == BUTTON_LEFT_MASK && !(GPIOB->PDIR & BUTTON_LEFT_MASK)) {
		flash();
	}
	else if (PORTB->ISFR == BUTTON_RIGHT_MASK && !(GPIOB->PDIR & BUTTON_RIGHT_MASK)) {
		beep();
	}
